...
from tabulate import tabulate #for left align
...
def get_filters():
    ...
    while True:
        try:
            month = input('What month? enter the number for the month, eg 1 for January, 6 for June. Or \'all\' for no filter. \n')
    ...
    while True:
        try:
            day = input('What day of week? enter the number for the weekday where Monday is 1 and Sunday 7. Or \'all\' for no filter. \n')
...
def station_stats(df):
    ...
    print(bcolors.BOLD + f'Most frequent combination of start and end station was: ' + bcolors.ENDC + f'{start} - {end}')
